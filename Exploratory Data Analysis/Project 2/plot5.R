...
NEIMotorVehicle <- filter(NEIMotorVehicle,fips == "24510")
NEIMotorVehicle <- group_by(NEIMotorVehicle,year)
NEIMotorVehicle <- summarise(NEIMotorVehicle, PM = sum(Emissions))

w <- qplot(year,PM, data = NEIMotorVehicle, geom = c("point","smooth"))
